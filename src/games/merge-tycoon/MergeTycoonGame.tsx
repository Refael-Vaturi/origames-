...
import { MergeTycoonGlyph } from "./MenuArt";
      ...
      <AnimatePresence>
        {phase === "menu" && (
          <motion.div
            ...
            style={{ background: "linear-gradient(180deg, #1a1206 0%, #0b0803 60%)" }}
          >
            <div className="max-w-sm w-full mx-auto min-h-full flex flex-col justify-center text-white py-8">
              <MergeTycoonGlyph className="w-32 h-[76px] mx-auto" />
              <h1 className="text-4xl font-display font-black text-center -mt-2">
                <span className="text-amber-300">Merge</span> <span className="text-orange-500">Tycoon</span>
              </h1>
              <p className="text-sm text-white/60 text-center mt-3">
                {isFirstVisit ? t("mergeTycoon.intro") : t("mergeTycoon.tagline")}
              </p>
              ...
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} className="mt-6">
                <Button
                  size="lg"
                  className="w-full bg-orange-500 hover:bg-orange-400 text-white font-display font-bold text-base shadow-[0_0_40px_rgba(251,146,60,0.4)]"
                  onClick={startGame}
                >
                  {t("mergeTycoon.start")}
                </Button>
              ...
              <div className="mt-6 border-t border-amber-400/10 pt-4">
        ...
        {phase === "gameover" && (
          ...
          >
            <div className="max-w-sm w-full mx-auto min-h-full flex flex-col justify-center text-white py-8 text-center">
              ...
              >
                <MergeTycoonGlyph className="w-24 h-[58px]" />
              </motion.div>
              <h2 className="text-2xl font-display font-bold">{t("mergeTycoon.gameOver")}</h2>
              <div className="text-5xl font-display font-black tabular-nums text-orange-400 mt-1">
                ${finalScore}
              </div>
              ...
              <div className="flex gap-2 mt-5">
                ...
                <motion.div className="flex-1" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.96 }}>
                  <Button className="w-full bg-orange-500 hover:bg-orange-400 text-white font-bold" onClick={startGame}>
                    {t("arcade.retry")}
                  </Button>
                </motion.div>
              </div>
