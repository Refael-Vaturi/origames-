...
import { isAndroidWebView, isInWebView, openOAuth } from "@/lib/webview";
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
      ...
      >
        <div className="bg-card rounded-3xl p-8 shadow-card">
          ...
          {/* OAuth buttons — hidden inside plain Android WebView (Google blocks it) */}
          {!androidWebView && (
            <div className="space-y-3 mb-6">
              <button
                onClick={handleGoogleSignIn}
                disabled={loading}
                className="w-full h-12 rounded-2xl border-2 border-input bg-background font-body text-sm text-foreground flex items-center justify-center gap-3 hover:bg-muted transition-colors disabled:opacity-50 active:scale-[0.97]"
              >
          ...
          {androidWebView && (
            <div className="mb-6 rounded-2xl border border-border bg-muted/50 p-3 text-xs text-muted-foreground font-body">
              {t("auth.inAppNotice") || "You're inside the Ori Games app — sign in with username or email below."}
            </div>
          )}
